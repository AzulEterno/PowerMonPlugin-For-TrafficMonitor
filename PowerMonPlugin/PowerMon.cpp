...
void PowerMon::DataRequired()
{
	...
	//MessageBox(NULL, L"准备尝试获取电池功耗数据代码", L"提示", MB_OK);

	bool result = do_update_info();

	if (!result && this->is_dbg) {
		MessageBox(NULL, (const wchar_t*)m_tooltip_info.c_str(), L"Tip", MB_OK);
	}
}
